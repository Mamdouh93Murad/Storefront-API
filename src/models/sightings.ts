...
export class sightingsStore {
  ...
  async create (
    ...
    try {
      ...
      const sql =
        'INSERT INTO sightings (name, description, number, user_id, region_id, category_id) VALUES ($1, $2, $3, (SELECT id FROM users WHERE name=($4)), (SELECT id FROM regions WHERE name=($5)), (SELECT id FROM categories WHERE name=($6))) RETURNING *'
      const result = await conn.query(sql, [
        s.name,
        s.description,
        s.number,
  ...
  async update (id: number, s: sighting): Promise<sighting> {
    try {
      ...
      const conn = await client.connect()
      const sql =
        'UPDATE sightings SET (name, description, number) = ($2, $3, $4) WHERE id=($1) RETURNING *'
      const result = await conn.query(sql, [
        id,
        s.name,
        s.description,
